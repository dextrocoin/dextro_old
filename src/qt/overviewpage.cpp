...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::DXO)
    {
    }
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeDextroAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDextroAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeDextroAmount = strAnonymizeDextroAmount.remove(strAnonymizeDextroAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDextroAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeDextroAmount * COIN) nMaxToAnonymize = nAnonymizeDextroAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeDextroAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeDextroAmount));
        strAnonymizeDextroAmount = strAnonymizeDextroAmount.remove(strAnonymizeDextroAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDextroAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
