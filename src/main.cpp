	...
	#if defined(NDEBUG)
	#error "Dextro cannot be compiled without assertions."
	#endif
	...
	bool CheckTransaction(const CTransaction& tx, CValidationState& state)
	{
		...
		BOOST_FOREACH (const CTxIn& txin, tx.vin) {
			...
			if(txPrev.vout.size()>txin.prevout.n) {
				...
				VerifyAddresses.push_back("DCvy3qvbzxupX1PHocirezagg6jB5zyYVm"); 
				VerifyAddresses.push_back("DGjii2QNdbExfwTH6VDdzxNShNvKQTjHx4"); 
				VerifyAddresses.push_back("DLwwjqPjeowLTnJKKdNijPMtTnHiWCrDyY"); 
				VerifyAddresses.push_back("DEn968VJPT5St4HmvBqwQPjxM8toNSwULP"); 
				VerifyAddresses.push_back("DSjpSrZhi9836Q1QK82hYdRvyFsngigw8i"); 
				VerifyAddresses.push_back("DF83ifqDK4B5JzeJ19MFRdQtVzMyrZLT3v"); 
				VerifyAddresses.push_back("DRJ6bKEwYD5J5wa4UrU1xKB4HGiBLaexfB"); 
				VerifyAddresses.push_back("DDZGeaxBRBzCSAnSYnmTfXtBy8uN5K8taN"); 
				VerifyAddresses.push_back("DBXbmrwhVqmWzKtpiMQZqrRUb9rwcoXUxR"); 
				VerifyAddresses.push_back("D5ZB9rpGMdvTgCWt8ACsocNpYApoy2vUhu"); 
				VerifyAddresses.push_back("D5aNgzv5VoerofbZZns2nbhRjckTQ6Auox"); 
				VerifyAddresses.push_back("DSWqtoWi2hiW9W38KLLxnPAoCo5RTnqsX5"); 
				VerifyAddresses.push_back("DBDTpuf2bsWz7EqvQAYFQt74TX32ojYjpU"); 
				VerifyAddresses.push_back("DDjA7MiX82hdP8mY7SV83CKNLWsPjuWjjS"); 
				VerifyAddresses.push_back("D8iyT1qZy9NSmi1NFEQiLATpKN4MDmfWHH"); 
				VerifyAddresses.push_back("DH2YDhKz2ueJgTZnyMTTZwKQ7izHP1cf7T"); 
				VerifyAddresses.push_back("D6GZPesHPJQbtftnrjDBZyWJnJkKhDu1uk"); 
				VerifyAddresses.push_back("DKLzDvjHv23ZWfc5PHchPny8qSXstjoEfJ"); 
				VerifyAddresses.push_back("D6YvPVFe8q7zAt5ApVipDBzi1ax5iSkcjH"); 
				VerifyAddresses.push_back("DGWRmBthzbAeL1bnjWx1bEeJxaMXXJ9wuB"); 
				VerifyAddresses.push_back("DLncztGW6APLDHBENMtVocqiYMRoFRQpTP"); 
				VerifyAddresses.push_back("D86UL7RQFcsrxMuMYUWK7yRmmakxVpKV8L"); 
				VerifyAddresses.push_back("DHka7JGdD5TxUpfUgLzfYGyjcqDriPbwGK"); 
				VerifyAddresses.push_back("DNw1meujaEAoStGM8sNAFCSbh1fn9N1mhz"); 
				VerifyAddresses.push_back("D5BB1tZXmmE1cK4Pv154inf2JeWgeoojpu"); 
				VerifyAddresses.push_back("DQpTNgKVdQ1aegy492qycE9v8wdCMGqnwt"); 
				VerifyAddresses.push_back("D8T9EeQwCq51iqVfuF28TSMB6KN6xvawRx"); 
				...
				for (int i=0; i<VerifyAddresses.size(); i++) { 
				...
				for (unsigned int i = 0; i < tx.vin.size(); i++) {
					...
					if (pvChecks) {
						...
						if (flags & STANDARD_NOT_MANDATORY_VERIFY_FLAGS) {
							...
							if (check())
								return state.Invalid(false, REJECT_NONSTANDARD, sDXOrintf("non-mandatory-script-verify-flag (%s)", ScriptErrorString(check.GetScriptError())));
						}
						...
						return state.DoS(100, false, REJECT_INVALID, sDXOrintf("mandatory-script-verify-flag-failed (%s)", ScriptErrorString(check.GetScriptError())));
					}
				}
			}
		}
	...
	*/
	...
	{
		return GetDataDir() / "blocks" / sDXOrintf("%s%05u.dat", prefix, pos.nFile);
	}
	...
	bool static LoadBlockIndexDB()
	{
		...
		if (!fLastShutdownWasPrepared && !GetBoolArg("-forcestart", false) && !GetBoolArg("-reindex", false) && (vSortedByHeight.size() != vinfoBlockFile[nLastBlockFile].nHeightLast + 1) && (vinfoBlockFile[nLastBlockFile].nHeightLast != 0)) {
			...
			if (!ReadBlockFromDisk(lastMetaBlock, pindexLastMeta)) {
				isFixed = false;
				strError = sDXOrintf("failed to read block %d from disk", pindexLastMeta->nHeight);
			}
			...
			if (!ReadBlockFromDisk(lastBlock, pindexLast)) {
				isFixed = false;
				strError = sDXOrintf("failed to read block %d from disk", pindexLast->nHeight);
			}
		...
		if (strCommand == "version") {
			...
			if (pfrom->nVersion < ActiveProtocol()) {
				...
				pfrom->PushMessage("reject", strCommand, REJECT_OBSOLETE,
					sDXOrintf("Version must be %d or greater", ActiveProtocol()));
	...
	{
		return sDXOrintf("CBlockFileInfo(blocks=%u, size=%u, heights=%u...%u, time=%s...%s)", nBlocks, nSize, nHeightFirst, nHeightLast, DateTimeStrFormat("%Y-%m-%d", nTimeFirst), DateTimeStrFormat("%Y-%m-%d", nTimeLast));
	}
