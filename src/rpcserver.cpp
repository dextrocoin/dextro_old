...
//! These are created by StarDXOCThreads, destroyed in StopRPCThreads
    ...
    bool fAllowNull)
{
    ...
    BOOST_FOREACH (Value_type t, typesExpected) {
        ...
        if (!((v.type() == t) || (fAllowNull && (v.type() == null_type)))) {
            string err = sDXOrintf("Expected type %s, got %s",
    ...
    bool fAllowNull)
{
    BOOST_FOREACH (const PAIRTYPE(string, Value_type) & t, typesExpected) {
        ...
        if (!fAllowNull && v.type() == null_type)
            throw JSONRPCError(RPC_TYPE_ERROR, sDXOrintf("Missing %s", t.first));

        if (!((v.type() == t.second) || (fAllowNull && (v.type() == null_type)))) {
            string err = sDXOrintf("Expected type %s for %s, got %s",
...
string CRPCTable::help(string strCommand) const
{
    ...
    if (strRet == "")
        strRet = sDXOrintf("help: unknown command: %s\n", strCommand);
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* Dextro features */
        {"dextro", "masternode", &masternode, true, true, false},
        {"dextro", "masternodelist", &masternodelist, true, true, false},
        {"dextro", "mnbudget", &mnbudget, true, true, false},
        {"dextro", "mnbudgetvoteraw", &mnbudgetvoteraw, true, true, false},
        {"dextro", "mnfinalbudget", &mnfinalbudget, true, true, false},
        {"dextro", "mnsync", &mnsync, true, true, false},
        {"dextro", "spork", &spork, true, true, false},
#ifdef ENABLE_WALLET
        {"dextro", "obfuscation", &obfuscation, false, false, true}, /* not threadSafe because of SendMoney */
...
void StarDXOCThreads()
{
        ...
        BOOST_FOREACH (string strAllow, vAllow) {
            ...
            if (!subnet.IsValid()) {
                uiInterface.ThreadSafeMessageBox(
                    sDXOrintf("Invalid -rpcallowip subnet specification: %s. Valid are a single IP (e.g. 1.2.3.4), a network/netmask (e.g. 1.2.3.4/255.255.255.0) or a network/CIDR (e.g. 1.2.3.4/24).", strAllow),
    ...
    if (((mapArgs["-rpcpassword"] == "") ||
        ...
        uiInterface.ThreadSafeMessageBox(sDXOrintf(
                                             _("To use dextrod, or the -server option to dextro-qt, you must set an rpcpassword in the configuration file:\n"
                                               "%s\n"
    ...
    } else if (mapArgs.count("-rpcbind")) // Specific bind address
    {
        BOOST_FOREACH (const std::string& addr, mapMultiArgs["-rpcbind"]) {
            try {
                ...
                uiInterface.ThreadSafeMessageBox(
                    sDXOrintf(_("Could not parse -rpcbind value %s as network address"), addr),
                    "", CClientUIInterface::MSG_ERROR);
                StartShutdown();
                return;
            }
    ...
    BOOST_FOREACH (const ip::tcp::endpoint& endpoint, vEndpoints) {
        try {
            ...
            strerr = sDXOrintf(_("An error occurred while setting up the RPC address %s port %u for listening: %s"), straddress, endpoint.port(), e.what());
        }
    }
...
void SeDXOCWarmupStatus(const std::string& newStatus)
{
    LOCK(cs_rpcWarmup);
    rpcWarmupStatus = newStatus;
}

void SeDXOCWarmupFinished()
{
    LOCK(cs_rpcWarmup);
    assert(fRPCInWarmup);
    fRPCInWarmup = false;
...
{
    return "> dextro-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(string methodname, string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:39321/\n";
}
