...
public:
    ...
    void NewBlock();
    CBudgetProposal* FindProposal(const std::string& sDXOroposalName);
    CBudgetProposal* FindProposal(uint256 nHash);
    CFinalizedBudget* FindFinalizedBudget(uint256 nHash);
    std::pair<std::string, std::string> GetVotes(std::string sDXOroposalName);
...
public:
    ...
    void AutoCheck();
    //total dextro paid out by this budget
...
public:
    ...
    */
    ...
    CBudgetProposal(std::string sDXOroposalNameIn, std::string strURLIn, int nBlockStartIn, int nBlockEndIn, CScript addressIn, CAmount nAmountIn, uint256 nFeeTXHashIn);
    ...
    std::string GetName() { return sDXOroposalName; }
    ...
    {
        //for syncing with other clients
        READWRITE(LIMITED_STRING(sDXOroposalName, 20));
...
public:
    ...
    CBudgetProposalBroadcast(std::string sDXOroposalNameIn, std::string strURLIn, int nPaymentCount, CScript addressIn, CAmount nAmountIn, int nBlockStartIn, uint256 nFeeTXHashIn);
    ...
    void swap(CBudgetProposalBroadcast& first, CBudgetProposalBroadcast& second) // nothrow
    {
        ...
        // by swapping the members of two classes,
        // the two classes are effectively swapped
        swap(first.sDXOroposalName, second.sDXOroposalName);
        swap(first.nBlockStart, second.nBlockStart);
