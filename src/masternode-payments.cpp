	...
	bool IsBlockValueValid(const CBlock& block, int64_t nExpectedValue)
	{
		...
		if (!masternodeSync.IsSynced()) { //there is no budget data to use to check anything
			...
			if (nHeight % GetBudgetPaymentCycleBlocks() < 100) {
				...
				if (block.vtx[T].GetValueOut() > nExpectedValue) return false;
			}
		} else { // we're synced and have data so check the budget schedule

			//are these blocks even enabled
			if (!IsSporkActive(SPORK_13_ENABLE_SUPERBLOCKS)) {
				return block.vtx[T].GetValueOut() <= nExpectedValue;
			}
	...
	bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
	{
		...
		BOOST_FOREACH (CMasternodePayee& payee, vecPayments) {
			...
			if (payee.nVotes >= MNPAYMENTS_SIGNATURES_REQUIRED) {
				...
				if (sDXOayeesPossible == "") {
					sDXOayeesPossible += address2.ToString();
				} else {
					sDXOayeesPossible += "," + address2.ToString();
				}
			}
	...
	bool CMasternodePaymentWinner::IsValid(CNode* pnode, std::string& strError)
	{
		...
		if (!pmn) {
			strError = sDXOrintf("Unknown Masternode %s", vinMasternode.prevout.ToStringShort());
		...
		if (pmn->protocolVersion < ActiveProtocol()) {
			strError = sDXOrintf("Masternode protocol too old %d - req %d", pmn->protocolVersion, ActiveProtocol());
			LogPrintf("CMasternodePaymentWinner::IsValid - %s\n", strError);
			return false;
		}
		...
		if (n > MNPAYMENTS_SIGNATURES_TOTAL) {
			...
			if (n > MNPAYMENTS_SIGNATURES_TOTAL * 2) {
				strError = sDXOrintf("Masternode not in the top %d (%d)", MNPAYMENTS_SIGNATURES_TOTAL, n);
				LogPrintf("CMasternodePaymentWinner::IsValid - %s\n", strError);
