...
bool isValidPassphrase(QString sDXOassphrase, QString& strInvalid)
{
    for (int i = 0; i < sDXOassphrase.size(); i++) {
        if (!validChar.contains(sDXOassphrase[i], Qt::CaseSensitive)) {
            if (QString("\"'").contains(sDXOassphrase[i]))
                continue;

            strInvalid = sDXOassphrase[i];
            return false;
        }
    }
...
void Bip38ToolDialog::on_encryptKeyButton_ENC_clicked()
{
    ...
    QString qsDXOassphrase = ui->passphraseIn_ENC->text();
    QString strInvalid;
    if (!isValidPassphrase(qsDXOassphrase, strInvalid)) {
    ...
    std::string encryptedKey = BIP38_Encrypt(addr.ToString(), qsDXOassphrase.toStdString(), key.GetPrivKey_256());
...
{
    string sDXOassphrase = ui->passphraseIn_DEC->text().toStdString();
