...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total dextro balance of the wallet\n"
            "  \"obfuscation_balance\": xxxxxx, (numeric) the anonymized dextro balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in dextro/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in dextro/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"dextroaddress\"\n"
            "\nReturn information about the given dextro address.\n"
            "\nArguments:\n"
            "1. \"dextroaddress\"     (string, required) The dextro address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"dextroaddress\", (string) The dextro address validated\n"
...
CScript _createmultisig_redeemScript(const Array& params)
{
    ...
    if ((int)keys.size() < nRequired)
        throw runtime_error(
            sDXOrintf("not enough keys supplied "
...
#ifdef ENABLE_WALLET
        // Case 1: Dextro address and we have full public key:
        CBitcoinAddress address(ks);
        if (pwalletMain && address.IsValid()) {
            CKeyID keyID;
            if (!address.GetKeyID(keyID))
                throw runtime_error(
                    sDXOrintf("%s does not refer to a key", ks));
            CPubKey vchPubKey;
            if (!pwalletMain->GetPubKey(keyID, vchPubKey))
                throw runtime_error(
                    sDXOrintf("no full public key for address %s", ks));
    ...
    if (result.size() > MAX_SCRIPT_ELEMENT_SIZE)
        throw runtime_error(
            sDXOrintf("redeemScript exceeds size limit: %d > %d", result.size(), MAX_SCRIPT_ELEMENT_SIZE));
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are dextro addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) dextro address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"dextroaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"dextroaddress\"  (string, required) The dextro address to use for the signature.\n"
